...
public class RoundRobinHandler {
  ...
  private final Map<Team, RoundRobinHandler.TeamStatistics> teamStatsMap = new HashMap<Team, RoundRobinHandler.TeamStatistics>();
  ...
  public void handleTournament(final Tournament tournament) {
    ...
    final Consumer<Team> _function = (Team team) -> {
      ...
      RoundRobinHandler.TeamStatistics _teamStatistics = new RoundRobinHandler.TeamStatistics(_name_1);
  ...
  public void updateTeamStatistics(final Team team, final int goalsScored, final int goalsConceded, final int pointsForWin, final int pointsForDraw, final int pointsForLoss) {
    final RoundRobinHandler.TeamStatistics stats = this.teamStatsMap.get(team);
  ...
  public void generateTournamentResults() {
    InputOutput.<String>println("\nFinal Standings:");
    Collection<RoundRobinHandler.TeamStatistics> _values = this.teamStatsMap.values();
    final ArrayList<RoundRobinHandler.TeamStatistics> sortedTeams = new ArrayList<RoundRobinHandler.TeamStatistics>(_values);
    final Comparator<RoundRobinHandler.TeamStatistics> _function = (RoundRobinHandler.TeamStatistics t1, RoundRobinHandler.TeamStatistics t2) -> {
      return Integer.compare(t2.points, t1.points);
    };
    Collections.<RoundRobinHandler.TeamStatistics>sort(sortedTeams, _function);
    final Consumer<RoundRobinHandler.TeamStatistics> _function_1 = (RoundRobinHandler.TeamStatistics stats) -> {
